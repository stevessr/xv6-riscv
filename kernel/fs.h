...
#define NDIRECT 12                       // 直接数据块指针的数量
#define NINDIRECT (BSIZE / sizeof(uint)) // 间接数据块指针的数量
...
{
  short type;              // 文件类型
  short major;             // 主设备号 (仅当类型为T_DEVICE时有效)
  short minor;             // 次设备号 (仅当类型为T_DEVICE时有效)
  short nlink;             // 连接到此inode的文件系统链接数
  uint size;               // 文件大小 (字节)
